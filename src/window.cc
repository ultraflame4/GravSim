...
void Window::static_framebuffer_size_callback(GLFWwindow *window, int width,
                                              int height) {
    windows_list[window]->framebuffer_size_callback(window, width, height);
}

void Window::static_key_callback(GLFWwindow *window, int key, int scancode,
                                 int action, int mods) {
    windows_list[window]->OnInput(key, action, mods);
}

void Window::static_mousebtn_callback(GLFWwindow *window, int key, int action,
                                      int mods) {
    windows_list[window]->OnInput(key, action, mods);
}

void Window::static_scroll_callback(GLFWwindow *window, double xoffset,
                                    double yoffset) {
...
GLenum CheckGLErrors_(const char *file, int line) {
    ...
    while ((errorCode = glGetError()) != GL_NO_ERROR) {
        ...
        logger->error("Error code {} : {} at \"{}\" line {}", errorCode, error,
                      file, line);
    }
    return errorCode;
}